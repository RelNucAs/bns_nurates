...
//.......Given the parameter eta the subroutine returns the integration extremes for GFF
          ...
          csi = (1./sigma)*log(1.+exp(sigma*(eta-d)));
...
//.......Given the parameter eta the subroutine returns the integration extremes for the first eta derivative of GFF
          ...
          csi = (1./sigmae)*log(1.+exp(sigmae*(eta-de)));
...
double compute_res(double eta, double theta, float k, gsl_integration_fixed_workspace *w_leg, gsl_integration_fixed_workspace *w_lag) {
        double r1,r2,r3,r4;
...
double compute_res_ed(double eta, double theta, float k, gsl_integration_fixed_workspace *w_leg, gsl_integration_fixed_workspace *w_lag) {
        double r1,r2,r3,r4;
        double s1 = 0., s2 = 0., s3 = 0.;
        struct FD_params p;
        gsl_function F;
	...
	F.function = &fermi_ed1;
        p = {k, eta, theta, 0., sqrt(s1)};
        F.params = &p;
        gsl_integration_fixed(&F, &r1, w_leg);

        F.function = &fermi_ed2;
        p = {k, eta, theta, s1, s2};
        F.params = &p;
        gsl_integration_fixed(&F, &r2, w_leg);
