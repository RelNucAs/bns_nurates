...
static MyQuadrature quadrature_default = {.type=kGauleg, .dim=1, .nx=20, .ny=1, .nz=1, .alpha=0., .x1=0., .x2=1., .y1=-42., .y2=-42., .z1=-42., .z2=-42.};
...
struct OpacityParams {
  ...
  bool neglect_blocking; // flag for neglecting blocking factor of antineutrino in pair (nu + anu) processes
};
...
__attribute__((unused))
static OpacityParams opacity_params_default_none = {.use_dU = false, .use_dm_eff = false, .use_WM_ab = false, .use_WM_sc = false, .neglect_blocking = false};
