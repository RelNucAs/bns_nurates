...
namespace weakrates
{
	...
	{
		double sigma_nun_abs = nb*sigma_o*((1+3*gA*gA)/4)*((e_nu+delta_np)/e_rm)*pow((1-pow((e_rm/(e_nu+delta_np)),2)),0.5)*B_nu(e_nu, e_nu_bar, mu_e, mu_np, temp, ne, pe, ee); //*Wm(e_nu)
	...
	{
		double sigma_nup_abs = nb*sigma_o*((1+3*gA*gA)/4)*((e_nu_bar-delta_np)/e_rm)*pow((1-pow((e_rm/(e_nu_bar-delta_np)),2)),0.5)*B_nu_bar(e_nu, e_nu_bar, mu_e, mu_np, temp, ne, pe, ee); //*Wm_bar(e_nu_bar)
