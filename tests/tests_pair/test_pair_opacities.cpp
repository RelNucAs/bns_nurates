...
int main() {
  ...
  for (int i = 0; i < 12; i++) {
    ...
    MyKernelQuantity result = PairOpacitiesFermi(&quad, &my_eos_params, &my_kernel_params);

    printf("%0.16e %.16e %0.16e %0.16e %0.16e\n", data_albino[i][0], data_albino[i][1], result.em_x, data_albino[i][2], 1./result.abs_e);
  }
