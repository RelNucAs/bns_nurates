...
void generate_comparison_data(const bool use_dU) {
  ...
  while (fgets(line, sizeof(line), fptr_in) != NULL) {
    ...
    sscanf(line,
           ...
           &h_zone(i),
           &h_r(i), &h_rho(i), &h_temp(i), &h_ye(i), &h_mu_e(i), &h_mu_hat(i), &h_yh(i), &h_ya(i), &h_yp(i),
           &h_yn(i), &h_du(i), &h_em_nue(i), &h_ab_nue(i), &h_em_anue(i), &h_ab_anue(i), &h_w_nue(i),
           &h_w_anue(i), &h_r_is(i), &h_w0_n(i), &h_w0_p(i), &h_w1_n(i), &h_w1_p(i));
  ...
  Kokkos::View<double*, LayoutWrapper, HostMemSpace> h_out_em_id_nue("h_out_em_id_nue", n_data);
  Kokkos::View<double*, LayoutWrapper, HostMemSpace> h_out_abs_id_nue("h_out_abs_id_nue", n_data);
  Kokkos::View<double*, LayoutWrapper, HostMemSpace> h_out_em_id_anue("h_out_em_id_anue", n_data);
  Kokkos::View<double*, LayoutWrapper, HostMemSpace> h_out_abs_id_anue("h_out_abs_id_anue", n_data);
  Kokkos::View<double*, LayoutWrapper, DevMemSpace> out_em_id_nue("out_em_id_nue", n_data);
  Kokkos::View<double*, LayoutWrapper, DevMemSpace> out_abs_id_nue("out_abs_id_nue", n_data);
  Kokkos::View<double*, LayoutWrapper, DevMemSpace> out_em_id_anue("out_em_id_anue", n_data);
  Kokkos::View<double*, LayoutWrapper, DevMemSpace> out_abs_id_anue("out_abs_id_anue", n_data);

  Kokkos::parallel_for("loop_over_opacities_test_beta", Kokkos::RangePolicy<>(DevExeSpace(), 0, n_data - 1),
  KOKKOS_LAMBDA(const int &i) {
    // Opacity parameters (correction to rates)
  OpacityParams opacity_pars = {.use_dU = use_dU, .use_dm_eff = false, .use_WM_ab = true, .use_decay = false}; // WM correction turned on
  ...
  for (int i = 0; i < n_data; i++) {
    fprintf(fptr_out,
            "%d %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e\n",
            h_zone(i), h_r(i), h_rho(i), h_temp(i), h_ye(i), h_mu_e(i), h_mu_hat(i),
            h_yh(i), h_ya(i), h_yp(i), h_yn(i), h_du(i), h_em_nue(i), h_ab_nue(i),
            h_em_anue(i), h_ab_anue(i), h_out_em_id_nue(i), h_out_abs_id_nue(i) / kClight,
            h_out_em_id_anue(i), h_out_abs_id_anue(i) / kClight);
  }
