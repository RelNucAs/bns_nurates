...
{
double nb = rho/mn
double sigma_nun_abs = nb*sigma_o*((1+3*pow(gA,2))/4)*((e_nu+delta_np)/e_rm)*pow((1-pow((e_rm/(e_nu+delta_np)),2)),0.5)*Wm(e_nu)*B_nu(e_nu, mu_e, mu_np, temp)
...
{
double nb = rho/mn
double sigma_nup_abs = nb*sigma_o*((1+3*pow(gA,2))/4)*((e_nu_bar-delta_np)/e_rm)*pow((1-pow((e_rm/(e_nu_bar-delta_np)),2)),0.5)*Wm_bar(e_nu_bar)*B_nu_bar(e_nu_bar, mu_e, mu_np, temp)
...
double e_nu =;
double e_nu_bar = ;
nun = nu_n_abs(rho, temp, ye, e_nu);
nup = nu_p_abs(rho, temp, ye, e_nu_bar);
