 ...
 *
 * Note: For almost all practical cases, this routine should be called and not
 * the 1d routine!
 *
 * Generates 1d/2d/3d quadratures. Provide a quadrature structure with properly
 * populated metadata. This routine populates the quadrature points & weights
 * arrays with (nx + ny + nz) elements each.
 *
 * The first nx elements contain quadrature information for the x variable, the
 * next ny for the y variable and so on.
 *
 * For the 1d case, the quadrature arrays are of the length (nx + 2) each, with
 * the weight and point information in the y and z direction being populated
 * with 1.
 *
 * Similarly, for the 2d case, the quadrature arrays are of length (nx + ny + 1)
 * each.
 *
 * Inputs:
 *      quad: A MyQuadrature structure to hold quadratures.
...
void GaussLegendreMultiD(MyQuadrature* quad)
{
    ...
    // Note: first nx elements contain quadrature for x variable, then ny
    // elements for y variable and then nx for z
    quad->points = NULL;
    quad->w      = NULL;
    quad->points =
        (double*)malloc((quad->nx + quad->ny + quad->nz) * sizeof(double));
    quad->w =
        (double*)malloc((quad->nx + quad->ny + quad->nz) * sizeof(double));
...
void GaussLaguerre(MyQuadrature* quad)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (i == 0)
        {
            z = (1.0 + quad->alpha) * (3.0 + 0.92 * quad->alpha) /
                (1.0 + 2.4 * n + 1.8 * quad->alpha);
        }
        else if (i == 1)
        {
            z += (15.0 + 6.25 * quad->alpha) /
                 (1.0 + 0.9 * quad->alpha + 2.5 * n);
        }
        else
        {
            ai = i - 1;
            z += ((1.0 + 2.55 * ai) / (1.9 * ai) +
                  1.26 * ai * quad->alpha / (1.0 + 3.5 * ai)) *
                 (z - x[i - 2]) / (1.0 + 0.3 * quad->alpha);
        }
