...
void WMScatt(const double omega, double* R0, double* R1, const int reacflag) {
  ...
  *R0 = (cv*cv + 3.*ca*ca + 1.5*ehor*ehor*F2*F2 + 2.*ehor*ehor*cv*F2) / h0; //correction to zeroth coefficient
  *R1 = (cv*cv -    ca*ca -  2.*ehor*ehor*F2*F2 - 4.*ehor*ehor*cv*F2) / h1; //correction to first coefficient
