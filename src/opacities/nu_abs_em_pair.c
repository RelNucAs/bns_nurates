...
// Calculate the 4 integrands needed for computing pair opacities (assuming no
// dependence on mu' and phi') var[3]: [omega' mu' phi'] can in generally have 3
// variables but the kernel we are considering has already been integrated over
// mu' and phi' and therefore only depends on the anti-neutrino energy (nu') and
// EOS/Kernel parameters.
MyKernelQuantity PairEmissivityAbsorptivityIntegrandFermi(
    double* var, MyEOSParams* my_eos_params, MyKernelParams* my_kernel_params)
{

    my_kernel_params->pair_kernel_params.omega_prime = var[0];
    MyKernelQuantity pair_kernel = PairKernelsPhiMuIntegrated(
        my_eos_params, &my_kernel_params->pair_kernel_params);

    double fermi_e = FermiDistr(var[0], my_eos_params->temp,
                                my_eos_params->mu_e / my_eos_params->temp);
