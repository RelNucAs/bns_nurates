...
MyQuadratureIntegrand M1CoeffsDoubleIntegrand(double *var, void *p) {
  ...
  MyKernelOutput pair_kernels_m1 = {0}; //{.em_e = 0., .abs_e = 0., .em_x = 0., .abs_x = 0.};
  if (opacity_flags.use_pair) {
    ...
    pair_kernels_m1 = PairKernelsOptimized(&my_grey_opacity_params->eos_pars, &my_grey_opacity_params->kernel_pars.pair_kernel_params);
  }
  ...
  pro_term[id_nue] = (pair_kernels_m1.em[id_nue] + brem_kernels_m1.em[id_nue]) * (1. - g_nu_bar[id_anue]) + inelastic_kernels_m1.em[id_nue] * g_nu_bar[id_nue];
  pro_term[id_anue] = (pair_kernels_m1.em[id_anue] + brem_kernels_m1.em[id_anue]) * (1. - g_nu_bar[id_nue]) + inelastic_kernels_m1.em[id_anue] * g_nu_bar[id_anue];
  pro_term[id_nux] = (pair_kernels_m1.em[id_nux] + brem_kernels_m1.em[id_nux]) * (1. - g_nu_bar[id_anux]) + inelastic_kernels_m1.em[id_nux] * g_nu_bar[id_nux];
  pro_term[id_anux] = (pair_kernels_m1.em[id_anux] + brem_kernels_m1.em[id_anux]) * (1. - g_nu_bar[id_nux]) + inelastic_kernels_m1.em[id_anux] * g_nu_bar[id_anux];
  ...
  ann_term[id_nue] = (pair_kernels_m1.abs[id_nue] + brem_kernels_m1.abs[id_nue]) * g_nu_bar[id_anue] + inelastic_kernels_m1.abs[id_nue] * (1. - g_nu_bar[id_nue]);
  ann_term[id_anue] = (pair_kernels_m1.abs[id_anue] + brem_kernels_m1.abs[id_anue]) * g_nu_bar[id_nue] + inelastic_kernels_m1.abs[id_anue] * (1. - g_nu_bar[id_anue]);
  ann_term[id_nux] = (pair_kernels_m1.abs[id_nux] + brem_kernels_m1.abs[id_nux]) * g_nu_bar[id_anux] + inelastic_kernels_m1.abs[id_nux] * (1. - g_nu_bar[id_nux]);
  ann_term[id_anux] = (pair_kernels_m1.abs[id_anux] + brem_kernels_m1.abs[id_anux]) * g_nu_bar[id_nux] + inelastic_kernels_m1.abs[id_anux] * (1. - g_nu_bar[id_anux]);
...
MyQuadratureIntegrand M1CoeffsSingleIntegrandNotStimulated(double *var, void *p) {
  ...
  if (opacity_flags.use_abs_em == 1) {
    ...
    for (int idx = 0; idx < total_num_species; idx++) {
      ...
      n_integrand_2[idx] = nu * nu * g_nu[idx] * abs_em_beta.abs[idx]; // ab = ab (NOT stimulated absorption)
      e_integrand_2[idx] = nu * n_integrand_2[idx];
    }
  }
...
void ComputeM1DoubleIntegrandNotStimulated(MyQuadrature *quad_1d, GreyOpacityParams *grey_pars, double t, M1Matrix *out_n, M1Matrix *out_j) {
  ...
  double pro_term_ij[total_num_species] = {0}, pro_term_ji[total_num_species] = {0};
  double ann_term_ij[total_num_species] = {0}, ann_term_ji[total_num_species] = {0};

  OpacityParams opacity_pars = grey_pars->opacity_pars;

  M1Matrix beta, pair, brem, inel;

  //BetaOpacitiesTable(quad_1d, &my_grey_opacity_params->eos_pars, &grey_pars->opacity_pars, t, &beta);

  InitializeM1Matrix(&pair, n);
  if (grey_pars->opacity_flags.use_pair == 1) {
    PairOpacitiesTable(quad_1d, grey_pars, t, &pair);
  }

  InitializeM1MatrixSingleFlavor(&brem, n, 0);
  if (grey_pars->opacity_flags.use_brem == 1) {
    BremOpacitiesTable(quad_1d, grey_pars, t, &brem);
  }

  InitializeM1Matrix(&inel, n);
  if (grey_pars->opacity_flags.use_inelastic_scatt == 1) {
    InelasticOpacitiesTable(quad_1d, grey_pars, t, &inel);
  }
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int j = i; j < n; j++) {
      ...
      pro_term_ij[id_nue] = (pair.m1_mat_em[id_nue][i][j] + brem.m1_mat_em[0][i][j]) * (1. - g_nu[id_nue]) * block_factor_nu_bar[id_anue];
      pro_term_ij[id_anue] = (pair.m1_mat_em[id_anue][i][j] + brem.m1_mat_em[0][i][j]) * (1. - g_nu[id_anue]) * block_factor_nu_bar[id_nue];
      pro_term_ij[id_nux] = (pair.m1_mat_em[id_nux][i][j] + brem.m1_mat_em[0][i][j]) * (1. - g_nu[id_nux]) * block_factor_nu_bar[id_anux];
      pro_term_ij[id_anux] = (pair.m1_mat_em[id_anux][i][j] + brem.m1_mat_em[0][i][j]) * (1. - g_nu[id_anux]) * block_factor_nu_bar[id_nux];

      pro_term_ji[id_nue] = (pair.m1_mat_em[id_nue][j][i] + brem.m1_mat_em[0][j][i]) * (1. - g_nu_bar[id_nue]) * block_factor_nu[id_anue];
      pro_term_ji[id_anue] = (pair.m1_mat_em[id_anue][j][i] + brem.m1_mat_em[0][j][i]) * (1. - g_nu_bar[id_anue]) * block_factor_nu[id_nue];
      pro_term_ji[id_nux] = (pair.m1_mat_em[id_nux][j][i] + brem.m1_mat_em[0][j][i]) * (1. - g_nu_bar[id_nux]) * block_factor_nu[id_anux];
      pro_term_ji[id_anux] = (pair.m1_mat_em[id_anux][j][i] + brem.m1_mat_em[0][j][i]) * (1. - g_nu_bar[id_anux]) *  block_factor_nu[id_nux];

      ann_term_ij[id_nue] = (pair.m1_mat_ab[id_nue][i][j] + brem.m1_mat_ab[0][i][j]) * g_nu_bar[id_anue];
      ann_term_ij[id_anue] = (pair.m1_mat_ab[id_anue][i][j] + brem.m1_mat_ab[0][i][j]) * g_nu_bar[id_nue];
      ann_term_ij[id_nux] = (pair.m1_mat_ab[id_nux][i][j] + brem.m1_mat_ab[0][i][j]) * g_nu_bar[id_anux];
      ann_term_ij[id_anux] = (pair.m1_mat_ab[id_anux][i][j] + brem.m1_mat_ab[0][i][j]) * g_nu_bar[id_nux];

      ann_term_ji[id_nue] = (pair.m1_mat_ab[id_nue][j][i] + brem.m1_mat_ab[0][j][i]) * g_nu[id_anue];
      ann_term_ji[id_anue] = (pair.m1_mat_ab[id_anue][j][i] + brem.m1_mat_ab[0][j][i]) * g_nu[id_nue];
      ann_term_ji[id_nux] = (pair.m1_mat_ab[id_nux][j][i] + brem.m1_mat_ab[0][j][i]) * g_nu[id_anux];
      ann_term_ji[id_anux] = (pair.m1_mat_ab[id_anux][j][i] + brem.m1_mat_ab[0][j][i]) * g_nu[id_nux];

      for (int idx = 0; idx < total_num_species; idx ++) {
        pro_term_ij[idx] += inel.m1_mat_em[idx][i][j] * g_nu_bar[idx] * (1. - g_nu[idx]); // @TODO: check this!
        pro_term_ji[idx] += inel.m1_mat_em[idx][j][i] * g_nu[idx] * (1. - g_nu_bar[idx]);

        ann_term_ij[idx] += inel.m1_mat_ab[idx][i][j] * block_factor_nu_bar[idx];
        ann_term_ji[idx] += inel.m1_mat_ab[idx][j][i] * block_factor_nu[idx];

        out_n->m1_mat_em[idx][i][j] = nu * nu * nu_bar * nu_bar * pro_term_ij[idx];
        out_n->m1_mat_em[idx][j][i] = nu_bar * nu_bar * nu * nu * pro_term_ji[idx];

        out_n->m1_mat_ab[idx][i][j] = nu * nu * g_nu[idx] * nu_bar * nu_bar * ann_term_ij[idx];
        out_n->m1_mat_ab[idx][j][i] = nu_bar * nu_bar * g_nu_bar[idx] * nu * nu * ann_term_ji[idx];
      ...
      pro_term_ij[id_nue] = (pair.m1_mat_em[id_nue][n+i][n+j] + brem.m1_mat_em[0][n+i][n+j]) * (1. - g_nu[id_nue]) * block_factor_nu_bar[id_anue];
      pro_term_ij[id_anue] = (pair.m1_mat_em[id_anue][n+i][n+j] + brem.m1_mat_em[0][n+i][n+j]) * (1. - g_nu[id_anue]) * block_factor_nu_bar[id_nue];
      pro_term_ij[id_nux] = (pair.m1_mat_em[id_nux][n+i][n+j] + brem.m1_mat_em[0][n+i][n+j]) * (1. - g_nu[id_nux]) * block_factor_nu_bar[id_anux];
      pro_term_ij[id_anux] = (pair.m1_mat_em[id_anux][n+i][n+j] + brem.m1_mat_em[0][n+i][n+j]) * (1. - g_nu[id_anux]) * block_factor_nu_bar[id_nux];

      pro_term_ji[id_nue] = (pair.m1_mat_em[id_nue][n+j][n+i] + brem.m1_mat_em[0][n+j][n+i]) * (1. - g_nu_bar[id_nue]) * block_factor_nu[id_anue];
      pro_term_ji[id_anue] = (pair.m1_mat_em[id_anue][n+j][n+i] + brem.m1_mat_em[0][n+j][n+i]) * (1. - g_nu_bar[id_anue]) * block_factor_nu[id_nue];
      pro_term_ji[id_nux] = (pair.m1_mat_em[id_nux][n+j][n+i] + brem.m1_mat_em[0][n+j][n+i]) * (1. - g_nu_bar[id_nux]) * block_factor_nu[id_anux];
      pro_term_ji[id_anux] = (pair.m1_mat_em[id_anux][n+j][n+i] + brem.m1_mat_em[0][n+j][n+i]) * (1. - g_nu_bar[id_anux]) *  block_factor_nu[id_nux];

      ann_term_ij[id_nue] = (pair.m1_mat_ab[id_nue][n+i][n+j] + brem.m1_mat_ab[0][n+i][n+j]) * g_nu_bar[id_anue];
      ann_term_ij[id_anue] = (pair.m1_mat_ab[id_anue][n+i][n+j] + brem.m1_mat_ab[0][n+i][n+j]) * g_nu_bar[id_nue];
      ann_term_ij[id_nux] = (pair.m1_mat_ab[id_nux][n+i][n+j] + brem.m1_mat_ab[0][n+i][n+j]) * g_nu_bar[id_anux];
      ann_term_ij[id_anux] = (pair.m1_mat_ab[id_anux][n+i][n+j] + brem.m1_mat_ab[0][n+i][n+j]) * g_nu_bar[id_nux];

      ann_term_ji[id_nue] = (pair.m1_mat_ab[id_nue][n+j][n+i] + brem.m1_mat_ab[0][n+j][n+i]) * g_nu[id_anue];
      ann_term_ji[id_anue] = (pair.m1_mat_ab[id_anue][n+j][n+i] + brem.m1_mat_ab[0][n+j][n+i]) * g_nu[id_nue];
      ann_term_ji[id_nux] = (pair.m1_mat_ab[id_nux][n+j][n+i] + brem.m1_mat_ab[0][n+j][n+i]) * g_nu[id_anux];
      ann_term_ji[id_anux] = (pair.m1_mat_ab[id_anux][n+j][n+i] + brem.m1_mat_ab[0][n+j][n+i]) * g_nu[id_nux];

      for (int idx = 0; idx < total_num_species; idx ++) {
        pro_term_ij[idx] += inel.m1_mat_em[idx][n+i][n+j] * g_nu_bar[idx] * (1. - g_nu[idx]); // @TODO: check this!
        pro_term_ji[idx] += inel.m1_mat_em[idx][n+j][n+i] * g_nu[idx] * (1. - g_nu_bar[idx]);

        ann_term_ij[idx] += inel.m1_mat_ab[idx][n+i][n+j] * block_factor_nu_bar[idx];
        ann_term_ji[idx] += inel.m1_mat_ab[idx][n+j][n+i] * block_factor_nu[idx];
        ...
        out_n->m1_mat_ab[idx][n+i][n+j] = nu * nu * g_nu[idx] * nu_bar * nu_bar * ann_term_ij[idx];
        out_n->m1_mat_ab[idx][n+j][n+i] = nu_bar * nu_bar * g_nu_bar[idx] * nu * nu * ann_term_ji[idx];
    ...
    for (int j = 0; j < n; j++) {
      ...
      pro_term_ij[id_nue] = (pair.m1_mat_em[id_nue][i][n+j] + brem.m1_mat_em[0][i][n+j]) * (1. - g_nu[id_nue]) * block_factor_nu_bar[id_anue];
      pro_term_ij[id_anue] = (pair.m1_mat_em[id_anue][i][n+j] + brem.m1_mat_em[0][i][n+j]) * (1. - g_nu[id_anue]) * block_factor_nu_bar[id_nue];
      pro_term_ij[id_nux] = (pair.m1_mat_em[id_nux][i][n+j] + brem.m1_mat_em[0][i][n+j]) * (1. - g_nu[id_nux]) * block_factor_nu_bar[id_anux];
      pro_term_ij[id_anux] = (pair.m1_mat_em[id_anux][i][n+j] + brem.m1_mat_em[0][i][n+j]) * (1. - g_nu[id_anux]) * block_factor_nu_bar[id_nux];

      pro_term_ji[id_nue] = (pair.m1_mat_em[id_nue][n+j][i] + brem.m1_mat_em[0][n+j][i]) * (1. - g_nu_bar[id_nue]) * block_factor_nu[id_anue];
      pro_term_ji[id_anue] = (pair.m1_mat_em[id_anue][n+j][i] + brem.m1_mat_em[0][n+j][i]) * (1. - g_nu_bar[id_anue]) * block_factor_nu[id_nue];
      pro_term_ji[id_nux] = (pair.m1_mat_em[id_nux][n+j][i] + brem.m1_mat_em[0][n+j][i]) * (1. - g_nu_bar[id_nux]) * block_factor_nu[id_anux];
      pro_term_ji[id_anux] = (pair.m1_mat_em[id_anux][n+j][i] + brem.m1_mat_em[0][n+j][i]) * (1. - g_nu_bar[id_anux]) *  block_factor_nu[id_nux];

      ann_term_ij[id_nue] = (pair.m1_mat_ab[id_nue][i][n+j] + brem.m1_mat_ab[0][i][n+j]) * g_nu_bar[id_anue];
      ann_term_ij[id_anue] = (pair.m1_mat_ab[id_anue][i][n+j] + brem.m1_mat_ab[0][i][n+j]) * g_nu_bar[id_nue];
      ann_term_ij[id_nux] = (pair.m1_mat_ab[id_nux][i][n+j] + brem.m1_mat_ab[0][i][n+j]) * g_nu_bar[id_anux];
      ann_term_ij[id_anux] = (pair.m1_mat_ab[id_anux][i][n+j] + brem.m1_mat_ab[0][i][n+j]) * g_nu_bar[id_nux];

      ann_term_ji[id_nue] = (pair.m1_mat_ab[id_nue][n+j][i] + brem.m1_mat_ab[0][n+j][i]) * g_nu[id_anue];
      ann_term_ji[id_anue] = (pair.m1_mat_ab[id_anue][n+j][i] + brem.m1_mat_ab[0][n+j][i]) * g_nu[id_nue];
      ann_term_ji[id_nux] = (pair.m1_mat_ab[id_nux][n+j][i] + brem.m1_mat_ab[0][n+j][i]) * g_nu[id_anux];
      ann_term_ji[id_anux] = (pair.m1_mat_ab[id_anux][n+j][i] + brem.m1_mat_ab[0][n+j][i]) * g_nu[id_nux];

      for (int idx = 0; idx < total_num_species; idx ++) {
        pro_term_ij[idx] += inel.m1_mat_em[idx][i][n+j] * g_nu_bar[idx] * (1. - g_nu[idx]); // @TODO: check this!
        pro_term_ji[idx] += inel.m1_mat_em[idx][n+j][i] * g_nu[idx] * (1. - g_nu_bar[idx]);

        ann_term_ij[idx] += inel.m1_mat_ab[idx][i][n+j] * block_factor_nu_bar[idx];
        ann_term_ji[idx] += inel.m1_mat_ab[idx][n+j][i] * block_factor_nu[idx];
        ...
        out_n->m1_mat_ab[idx][i][n+j] = nu * nu * g_nu[idx] * nu_bar * nu_bar * ann_term_ij[idx];
        out_n->m1_mat_ab[idx][n+j][i] = nu_bar * nu_bar * g_nu_bar[idx] * nu * nu * ann_term_ji[idx];
...
void ComputeM1DoubleIntegrand(MyQuadrature *quad_1d, GreyOpacityParams *grey_pars, double t, M1Matrix *out_n, M1Matrix *out_j) {
  const int n = quad_1d->nx;
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int j = i; j < n; j++) {
      ...
      pro_term_ij[id_nue] = (pair.m1_mat_em[id_nue][i][j] + brem.m1_mat_em[0][i][j]) * block_factor_nu_bar[id_anue];
      pro_term_ij[id_anue] = (pair.m1_mat_em[id_anue][i][j] + brem.m1_mat_em[0][i][j]) * block_factor_nu_bar[id_nue];
      pro_term_ij[id_nux] = (pair.m1_mat_em[id_nux][i][j] + brem.m1_mat_em[0][i][j]) * block_factor_nu_bar[id_anux];
      pro_term_ij[id_anux] = (pair.m1_mat_em[id_anux][i][j] + brem.m1_mat_em[0][i][j]) * block_factor_nu_bar[id_nux];

      pro_term_ji[id_nue] = (pair.m1_mat_em[id_nue][j][i] + brem.m1_mat_em[0][j][i]) * block_factor_nu[id_anue];
      pro_term_ji[id_anue] = (pair.m1_mat_em[id_anue][j][i] + brem.m1_mat_em[0][j][i]) * block_factor_nu[id_nue];
      pro_term_ji[id_nux] = (pair.m1_mat_em[id_nux][j][i] + brem.m1_mat_em[0][j][i]) * block_factor_nu[id_anux];
      pro_term_ji[id_anux] = (pair.m1_mat_em[id_anux][j][i] + brem.m1_mat_em[0][j][i]) * block_factor_nu[id_nux];
      ...
      pro_term_ij[id_nue] = (pair.m1_mat_em[id_nue][n+i][n+j] + brem.m1_mat_em[0][n+i][n+j]) * block_factor_nu_bar[id_anue];
      pro_term_ij[id_anue] = (pair.m1_mat_em[id_anue][n+i][n+j] + brem.m1_mat_em[0][n+i][n+j]) * block_factor_nu_bar[id_nue];
      pro_term_ij[id_nux] = (pair.m1_mat_em[id_nux][n+i][n+j] + brem.m1_mat_em[0][n+i][n+j]) * block_factor_nu_bar[id_anux];
      pro_term_ij[id_anux] = (pair.m1_mat_em[id_anux][n+i][n+j] + brem.m1_mat_em[0][n+i][n+j]) * block_factor_nu_bar[id_nux];

      pro_term_ji[id_nue] = (pair.m1_mat_em[id_nue][n+j][n+i] + brem.m1_mat_em[0][n+j][n+i]) * block_factor_nu[id_anue];
      pro_term_ji[id_anue] = (pair.m1_mat_em[id_anue][n+j][n+i] + brem.m1_mat_em[0][n+j][n+i]) * block_factor_nu[id_nue];
      pro_term_ji[id_nux] = (pair.m1_mat_em[id_nux][n+j][n+i] + brem.m1_mat_em[0][n+j][n+i]) * block_factor_nu[id_anux];
      pro_term_ji[id_anux] = (pair.m1_mat_em[id_anux][n+j][n+i] + brem.m1_mat_em[0][n+j][n+i]) * block_factor_nu[id_nux];
      ...
      for (int idx = 0; idx < total_num_species; idx ++) {
        pro_term_ij[idx] += inel.m1_mat_em[idx][n+i][n+j] * g_nu_bar[idx];
        pro_term_ji[idx] += inel.m1_mat_em[idx][n+j][n+i] * g_nu[idx];
    ...
    for (int j = 0; j < n; j++) {
      ...
      pro_term_ij[id_nue] = (pair.m1_mat_em[id_nue][i][n+j] + brem.m1_mat_em[0][i][n+j]) * block_factor_nu_bar[id_anue];
      pro_term_ij[id_anue] = (pair.m1_mat_em[id_anue][i][n+j] + brem.m1_mat_em[0][i][n+j]) * block_factor_nu_bar[id_nue];
      pro_term_ij[id_nux] = (pair.m1_mat_em[id_nux][i][n+j] + brem.m1_mat_em[0][i][n+j]) * block_factor_nu_bar[id_anux];
      pro_term_ij[id_anux] = (pair.m1_mat_em[id_anux][i][n+j] + brem.m1_mat_em[0][i][n+j]) * block_factor_nu_bar[id_nux];

      pro_term_ji[id_nue] = (pair.m1_mat_em[id_nue][n+j][i] + brem.m1_mat_em[0][n+j][i]) * block_factor_nu[id_anue];
      pro_term_ji[id_anue] = (pair.m1_mat_em[id_anue][n+j][i] + brem.m1_mat_em[0][n+j][i]) * block_factor_nu[id_nue];
      pro_term_ji[id_nux] = (pair.m1_mat_em[id_nux][n+j][i] + brem.m1_mat_em[0][n+j][i]) * block_factor_nu[id_anux];
      pro_term_ji[id_anux] = (pair.m1_mat_em[id_anux][n+j][i] + brem.m1_mat_em[0][n+j][i]) * block_factor_nu[id_nux];
      ...
      for (int idx = 0; idx < total_num_species; idx ++) {
        pro_term_ij[idx] += inel.m1_mat_em[idx][i][n+j] * g_nu_bar[idx];
        pro_term_ji[idx] += inel.m1_mat_em[idx][n+j][i] * g_nu[idx];
...
M1Opacities ComputeM1OpacitiesNotStimulated(MyQuadrature *quad_1d, MyQuadrature *quad_2d, GreyOpacityParams *my_grey_opacity_params) {
  ...
  ComputeM1DoubleIntegrandNotStimulated(quad_1d, my_grey_opacity_params, s, &out_n, &out_j);
  MyQuadratureIntegrand n_integrals_2d = GaussLegendreIntegrate2DMatrix(quad_1d, &out_n, s);
  MyQuadratureIntegrand e_integrals_2d = GaussLegendreIntegrate2DMatrix(quad_1d, &out_j, s);

  M1Opacities m1_opacities;

  m1_opacities.eta_0[id_nue] = four_pi_hc3 * (four_pi_hc3 * n_integrals_2d.integrand[0] + integrals_1d.integrand[0]);
  m1_opacities.eta_0[id_anue] = four_pi_hc3 * (four_pi_hc3 * n_integrals_2d.integrand[1] + integrals_1d.integrand[1]);
  ...
  m1_opacities.kappa_0_a[id_nue] = four_pi_hc3 / (kClight * n[id_nue]) * (four_pi_hc3 * n_integrals_2d.integrand[4] + integrals_1d.integrand[2]);
  m1_opacities.kappa_0_a[id_anue] = four_pi_hc3 / (kClight * n[id_anue]) * (four_pi_hc3 * n_integrals_2d.integrand[5] + integrals_1d.integrand[3]);
  m1_opacities.kappa_0_a[id_nux] = four_pi_hc3_sqr / (kClight * n[id_nux]) * n_integrals_2d.integrand[6];
  m1_opacities.kappa_0_a[id_anux] = four_pi_hc3_sqr / (kClight * n[id_anux]) * n_integrals_2d.integrand[7];

  m1_opacities.eta[id_nue] = four_pi_hc3 * (four_pi_hc3 * e_integrals_2d.integrand[0] + integrals_1d.integrand[4]);
  m1_opacities.eta[id_anue] = four_pi_hc3 * (four_pi_hc3 * e_integrals_2d.integrand[1] + integrals_1d.integrand[5]);
  ...
  m1_opacities.kappa_a[id_nue] = four_pi_hc3 / (kClight * J[id_nue]) * (four_pi_hc3 * e_integrals_2d.integrand[4] + integrals_1d.integrand[6]);
  m1_opacities.kappa_a[id_anue] = four_pi_hc3 / (kClight * J[id_anue]) * (four_pi_hc3 * e_integrals_2d.integrand[5] + integrals_1d.integrand[7]);
  m1_opacities.kappa_a[id_nux] = four_pi_hc3_sqr / (kClight * J[id_nux]) * e_integrals_2d.integrand[6];
  m1_opacities.kappa_a[id_anux] = four_pi_hc3_sqr / (kClight * J[id_anux]) * e_integrals_2d.integrand[7];
...
M1Opacities ComputeM1Opacities(MyQuadrature *quad_1d, MyQuadrature *quad_2d, GreyOpacityParams *my_grey_opacity_params) {
  ...
  M1Matrix out_n, out_j;
  ComputeM1DoubleIntegrand(quad_1d, my_grey_opacity_params, s, &out_n, &out_j);
  MyQuadratureIntegrand n_integrals_2d = GaussLegendreIntegrate2DMatrix(quad_1d, &out_n, s);
  MyQuadratureIntegrand e_integrals_2d = GaussLegendreIntegrate2DMatrix(quad_1d, &out_j, s);
...
MyQuadratureIntegrand SpectralIntegrand(double *var, void *p) {
  ...
  if (opacity_flags.use_pair) {
    ...
    pair_kernels_m1 = PairKernelsOptimized(&my_eos_params, &my_grey_opacity_params->kernel_pars.pair_kernel_params);
  }
  ...
  pro_term[id_nue] = (pair_kernels_m1.em[id_nue] + brem_kernels_m1.em[id_nue]) * block_factor[id_anue];                                                                                   
  pro_term[id_anue] = (pair_kernels_m1.em[id_anue] + brem_kernels_m1.em[id_anue]) * block_factor[id_nue];
  pro_term[id_nux] = (pair_kernels_m1.em[id_nux] + brem_kernels_m1.em[id_nux]) * block_factor[id_anux];
  pro_term[id_anux] = (pair_kernels_m1.em[id_anux] + brem_kernels_m1.em[id_anux]) * block_factor[id_nux];
  ...
  ann_term[id_nue] = (pair_kernels_m1.abs[id_nue] + brem_kernels_m1.abs[id_nue]) * g_nu_bar[id_anue];
  ann_term[id_anue] = (pair_kernels_m1.abs[id_anue] + brem_kernels_m1.abs[id_anue]) * g_nu_bar[id_nue];
  ann_term[id_nux] = (pair_kernels_m1.abs[id_nux] + brem_kernels_m1.abs[id_nux]) * g_nu_bar[id_anux];
  ann_term[id_anux] = (pair_kernels_m1.abs[id_anux] + brem_kernels_m1.abs[id_anux]) * g_nu_bar[id_nux];

  //////////////////////////////////////////////
  ////// ONLY FOR COMPARISON WITH NULIB ////////
  //////////////////////////////////////////////
  if (false) {
    ann_term[id_nue] += pair_kernels_m1.em[id_nue] * g_nu_bar[id_anue] / g_nu[id_nue];
    ann_term[id_anue] += pair_kernels_m1.em[id_anue] * g_nu_bar[id_nue] / g_nu[id_anue];
    ann_term[id_nux] += pair_kernels_m1.em[id_nux] * g_nu_bar[id_anux] / g_nu[id_nux];
    ann_term[id_anux] += pair_kernels_m1.em[id_anux] * g_nu_bar[id_nux] / g_nu[id_anux];
  }
  ////////////////////////////////////////////// 
...
SpectralOpacities ComputeSpectralOpacitiesNotStimulatedAbs(const double nu, MyQuadrature *quad_1d, GreyOpacityParams *my_grey_opacity_params) {
  ...
  if (my_grey_opacity_params->opacity_flags.use_abs_em) {
    abs_em_beta = AbsOpacity(nu, &my_grey_opacity_params->opacity_pars, &my_grey_opacity_params->eos_pars); // [s^-1]
  }
...
SpectralOpacities ComputeSpectralOpacitiesStimulatedAbs(const double nu, MyQuadrature *quad_1d, GreyOpacityParams *my_grey_opacity_params) {
  SpectralOpacities spec_opacs = ComputeSpectralOpacitiesNotStimulatedAbs(nu, quad_1d, my_grey_opacity_params);

  for (int idx = 0; idx < total_num_species; idx ++){
    spec_opacs.kappa[idx] = spec_opacs.j[idx] / kClight + spec_opacs.kappa[idx];
  }
