...
double PairTWithAlpha0(int l) {
  switch(l) {
    ...
    default:
      printf("PairTWithAlpha0 (kernel_pair.c): l = %d must be within 1 and 6\n", l);
      exit(EXIT_FAILURE);
  }
} 
...
double PairTFittedOld(int l, double alpha) {
  ...
  if (alpha < 15.) { 
    ...
    switch (l) {
      ...
      default:
        printf("PairTFitted (kernel_pair.c): l = %d must be within 1 and 6\n", l);
        exit(EXIT_FAILURE);
    }
  }
...
double PairTFitted(int l, double alpha) {
  const double a[6][13] = {{0.0009591818519410957, -0.007025445963539029, 0.02413571687574133, -0.05283857730366366, 0.08565184150347487, -0.1148974833969069, 0.14001269129489452, -0.16612181245182328, 0.19993311519859502, -0.2499952138069261, 0.333333164848656, -0.4999999980100303, 0.9999999999994182},
{9.806117191850437e-05, -0.0007303693955082037, 0.0025729046032377245, -0.005853104482681513, 0.010060162627142736, -0.014705257422400938, 0.02014664666291523, -0.027727326966761626, 0.039993773761235564, -0.062499552625431457, 0.11111109531105254, -0.24999999981287394, 0.999999999999945, },
{9.83660257000452e-06, -7.455399425901051e-05, 0.00026968069130985794, -0.0006390877179467623, 0.0011691212967386872, -0.0018705673951822446, 0.0028917745803742146, -0.0046250335703596395, 0.007999430192056203, -0.015624958913956509, 0.03703703558177793, -0.12499999998270364, 0.999999999999995},
{9.63655550546077e-07, -7.4501714680668755e-06, 2.775504190088471e-05, -6.878346378088752e-05, 0.00013456162975559815, -0.00023676741517985415, 0.00041435638829548566, -0.0007711865782589587, 0.0015999477373502997, -0.003906246205609882, 0.012345678877131755, -0.06249999999837849, 0.9999999999999992},
{9.440437566332406e-08, -7.417123619081298e-07, 2.838371714509535e-06, -7.350534216150028e-06, 1.5399946770677647e-05, -2.9877545939654987e-05, 5.931172050750023e-05, -0.00012856362411722033, 0.00031999523335656476, -0.0009765621364823982, 0.004115226323079645, -0.031249999999776252, 1.0000000000000002},
{1.1258637058816464e-08, -8.606136518301832e-08, 3.2215497280909555e-07, -8.319730818363639e-07, 1.8034586648497246e-06, -3.792399721761286e-06, 8.496951883750312e-06, -2.1433700743647905e-05, 6.400013840911159e-05, -0.00024414064372257427, 0.001371742113499251, -0.015625000000008864, 1.0000000000000002}};
...
void PairPsiOptimized(int l, double y, double z, double eta, double *psi_out) {
  ...
  for (int n = 3; n <= 2 * l + 5; n++) {
    ...
    //aux = PairG(n, 0, min_yz, eta, y, z) - PairG(n, max_yz, y + z, eta, y, z);
 ...
 */
void PairPhiOptimized(const double omega, const double omega_prime, int l, double eta, double temp, double *phi_out) {
  ...
  phi_out[1] = phi_prefactor * (kAlpha1[0] * kAlpha1[0] * pair_psi[1] + kAlpha2[0] * kAlpha2[0] * pair_psi[0]) / phi_denom;
  phi_out[2] = phi_prefactor * (kAlpha1[1] * kAlpha1[1] * pair_psi[0] + kAlpha2[1] * kAlpha2[1] * pair_psi[1]) / phi_denom;
  phi_out[3] = phi_prefactor * (kAlpha1[1] * kAlpha1[1] * pair_psi[1] + kAlpha2[1] * kAlpha2[1] * pair_psi[0]) / phi_denom;
...
MyKernelOutput PairKernelsOptimized(MyEOSParams *eos_pars, PairKernelParams *kernel_pars) {
  ...
  for (int idx = 0; idx < total_num_species; idx ++) {
    pair_kernel.abs[idx] = SafeExp((omega + omega_prime) / temp) * pair_kernel.em[idx];
  }
...
double PairF(int k, double eta, double x1) {
  ...
  if (eta < 0.) {
    double sum = 0.;
    for (int l = 0; l <= k; l++) {
      sum += PairTFitted(k + 1 - l, x1 - eta) * pow(x1, l) / tgamma(l + 1);
    }
    ...
    double sum_2 = 0.;
    for (int l = 0; l <= k; l++) {
      sum_2 += PairTFitted(k + 1 - l, x1 - eta) * pow(x1, l) / tgamma(l + 1);
    }

    result = pow(eta, k + 1.) / (k + 1.) + tgamma(k + 1) * (2.0 * sum + pow(-1., k) * PairTFitted(k + 1, eta) - sum_2);
    ...
    result = pow(eta, k + 1.) / (k + 1.) + tgamma(k + 1) * (pow(-1., k) * PairTFitted(k + 1, eta) - sum);

  } else {

    double sum = 0.;
    for (int l = 0; l <= k; l++) {
      sum += pow(-1., k - l) * PairTFitted(k + 1 - l, eta - x1) * pow(x1, l) / tgamma(l + 1);
    }
    result = pow(x1, k + 1.) / (k + 1.) + tgamma(k + 1) * (pow(-1., k) * PairTFitted(k + 1, eta) - sum);
